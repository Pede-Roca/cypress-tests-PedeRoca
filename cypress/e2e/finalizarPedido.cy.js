describe('Login Test', () => {
  it('should login with valid credentials', () => {
    ...
    cy.get('#_filtroPesquisa_12ack_1').type('feijão')
    cy.wait(1500)
    ...
    //---------------- Processo de finalização de compra --------------------------
     //proxima etapa
    cy.get('._btnNext_19ngf_143').click()
    cy.wait(2000)

    cy.get('._btnNext_19ngf_143').click()
    cy.wait(2000)

    // Seleciona a primeira opção de pagamento
    cy.get('input[name="pagamento"]').eq(0).click()
    cy.wait(1000)
    cy.get('._btnNext_19ngf_143').click()
    cy.wait(5000)
    ...
    cy.on('window:alert', (str) => {
        expect(str).to.include('Compra finalizada com sucesso')
    })
